...
function fixMarkdownFormatting(content: string): string {
  console.log('[fixMarkdownFormatting] Input content (first 300 chars):', content.substring(0, 300));
  let fixed = content;
  ...
  console.log('[fixMarkdownFormatting] After Fix 1 (heading on own line):', fixed.substring(0, 300));

  // Fix 2: Ensure blank line after heading (only for level-3 headings)
  // Pattern: ### Heading\nDescription → ### Heading\n\nDescription
  fixed = fixed.replace(/^(###\s+.+)$/gm, "$1\n");
  console.log('[fixMarkdownFormatting] After Fix 2 (blank line after heading):', fixed.substring(0, 300));

  // Fix 3: Remove leading space before "Key Highlights:"
  fixed = fixed.replace(/^\s+Key Highlights:/gm, "Key Highlights:");
  console.log('[fixMarkdownFormatting] After Fix 3 (remove leading space):', fixed.substring(0, 300));
  ...
  fixed = fixed.replace(/^(Key Highlights:\s*)$/gm, "$1\n").replace(/(-\s+[^-\n]+)(\s+-\s+)/g, "$1\n$2");
  console.log('[fixMarkdownFormatting] After Fix 4 (split bullet points):', fixed.substring(0, 300));

  // Fix 5: Add blank line before each new project heading (###)
  // Pattern: - Bullet\n### Next Project → - Bullet\n\n### Next Project
  fixed = fixed.replace(/(-\s+[^\n]+)\n(###\s+)/g, "$1\n\n$2");
  console.log('[fixMarkdownFormatting] After Fix 5 (blank line between projects):', fixed.substring(0, 300));
...
const AIMarkdownRenderer = memo(function AIMarkdownRenderer({
  ...
  console.log('[AIMarkdownRenderer] processedContent (first 300 chars):', processedContent.substring(0, 300));
